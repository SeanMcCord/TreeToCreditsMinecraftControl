...
export const tossEyeOfEnder = async (bot, mcData) => {
  ...
  if (!item) {
    logger.info({itemNotFoundInInventory: eyeOfEnder});
  ...
  logger.info('activate eye of ender');
  bot.activateItem();
...
export class StrongholdSensor {
  constructor(bot, mcData) {
    ...
    // logger.info({map: this._chunkMap});
  ...
  async _findStrongholdBlocksInChunk(chunkX, chunkZ) {
    ...
    logger.info({chunkPosition, countBLocksTime: performance.now() - chunkStartTime});
  }
      ...
      if (this._location.target == null || (!this._location.finalTarget)) {
        ...
        for (const key of keys) {
          if (portalRoomBlocks.has(key)) {
            ...
            // logger.info({closestPortalRoomBlock});
            if (closestPortalRoomBlock != null) {
              logger.info({foundFinalTarget: closestPortalRoomBlock});
              ignoreY = false;
              finalTarget = true;
        ...
        if (this._location.target == null || (!this._location.finalTarget && this._location.blockCount < blockCount) || finalTarget) {
          ...
          logger.info({locationUpdate: this._location});
        }
      }
      // logger.info({chunkX, chunkZ, blockCount, time: performance.now() - chunkStartTime, map: this._chunkMap.get(chunkPosition)});
    }
    logger.info({chunkPosition, updateTargetTime: performance.now() - chunkStartTime});
  }
...
export const placeEyesOfEnder = async (bot, mcData) => {
  ...
  for (const emptyPortalFrame of emptyPortalFrames) {
    logger.info({placeEyeOfEnderIn: emptyPortalFrame.position});
