...
export default async (bot, mcData) => {
  ...
  logger.info({main_work: {event: 'collect logs'}})
  try {
    await collectNblocks(bot, 3, log_block_ids, 256)
  } catch (e) {
    logger.info({main_work: {event: 'getting some dirt then will try again'}})
    await collectNblocks(bot, 10, [mcData.blocksByName.dirt.id], 256)
    await collectNblocks(bot, 3, log_block_ids, 256)
  }
  logger.info({main_work: {event: 'craft planks'}})
  await craftItems(bot, plank_item_ids, 1, null)
  logger.info({main_work: {event: 'craft craftingtable'}})
  ...
  logger.info({main_work: {event: 'create axe start'}})
  await craftItems(bot, [mcData.itemsByName.wooden_axe.id], 1, craftingTable)
  logger.info({main_work: {event: 'create axe done'}})
  ...
  try {
    ...
    logger.info({main_work: {event: 'can not get path to old crafting table, making another'}})
    await collectNblocks(bot, 1, log_block_ids, 256)
    await craftItems(bot, [mcData.itemsByName.crafting_table.id], 1, null)
  }

  logger.info({main_work: {event: 'create pickaxe start'}})
  await craftItems(bot, [mcData.itemsByName.wooden_pickaxe.id], 1, craftingTable)
  logger.info({main_work: {event: 'create pickaxe done'}})
  // take the crafting table with us
  await new Promise((resolve, reject) => {
    bot.collectBlock.collect(craftingTable, (err) => {
      if (err) {
        logger.info({main_work: {event: {failedToCollectCraftingTable: err}}})
        reject(err)
      } else {
        resolve()
      }
  ...
  await craftItems(bot, [mcData.itemsByName.stick.id], 1, null)
  // place the crafting table and use it
  logger.info({main_work: {event: 'place crafting table'}})
  ...
  logger.info({main_work: {event: 'get some stone'}})
  ...
  logger.info({main_work: {event: 'smelt start'}})
  ...
  if (cobbleCollectCount > 0) await collectNblocks(bot, cobbleCollectCount, cobbleDroppingBlockIds, 64)
  logger.info({main_work: {event: 'create stone shovel start'}})
  ...
  await new Promise((resolve, reject) => {
    bot.collectBlock.collect(furnace, (err) => {
      if (err) {
        logger.info({main_work: {event: {failedToCollectFurnace: err}}})
        reject(err)
      } else {
        resolve()
      }
  ...
  logger.info({main_work: {event: 'WOWOWOWOWOOWOWOW'}})
}
