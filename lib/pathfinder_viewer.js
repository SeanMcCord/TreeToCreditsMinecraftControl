...
export default (bot) => {
  ...
  const pathUpdateViewer = (pathResult) => {
    ...
    if (pathNew.length != pathMemo.length) {
      ...
      // logger.info({pathMemo})
    }
    // logger.info({botId: bot.username, pathfinderPath, pathMemo})
    ...
    const extractDangerBlocksFromNeoGoal = (goal) => {
      ...
      for (const blocks of goal._fireBallSensor.getDangerBlocksByFireBall().values()) {
        // logger.info({dangerBlocks: goal._dangerBlocks});
    ...
    // logger.info({botId: bot.username, toBreak })
    ...
    // logger.info({botId: bot.username, toPlace })
