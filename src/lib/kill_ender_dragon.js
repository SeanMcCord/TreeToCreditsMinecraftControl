...
export class DragonHeadFollower {
  constructor(bot, mcData) {
    ...
    this._perchDetect = (entity) => {
      ...
      logger.info({dragonPhase: entity.metadata[15]});
    }
    ...
    this._headMoved = async (entity) => {
      ...
      //logger.info({countEntityMoved: bot.listenerCount('entityMoved'), countEntityUpdate: bot.listenerCount('entityUpdate')});
      ...
      if (headOfBedBlock == null || footOfBedBlock == null) {
        ...
        // logger.info({time: performance.now() - startTime});
        return
      }
      ...
      // logger.info({dy, dxz});
      if (dy < 0.5) {
        logger.info({headAtHalfBlock: this._headPos});
      }
      if (this._interactingWithBed) {
        // logger.info({time: performance.now() - startTime});
        return;
      ...
      if (dy >= 0 && dy < 1 && dxz < 1) {
        logger.info({dy, dxz});
        logger.info('activate bed 55ish');
        try {
          if (this._interactingWithBed) {
            return;
          }
      ...
      if (dy >= 0 && dy < 0.5 && dxz < 1.6) {
        logger.info({dy, dxz});
        logger.info('activate bed 54ish');
        try {
          if (this._interactingWithBed) {
            return;
          }
      ...
      if (dy >= 0 && dy < 2 && dxz < 1.6) {
        logger.info({dy, dxz});
        logger.info('activate bed 51ish');
        try {
          if (this._interactingWithBed) {
            return;
          }
      ...
      if (dy >= 0 && dy < 0.86 && dxz < 8) {
        logger.info({dy, dxz});
        logger.info('activate bed take the L');
        try {
          if (this._interactingWithBed) {
            return;
          }
      ...
      if (dy >= 0 && dy < 0.47 && dxz < 15) {
        logger.info({dy, dxz});
        logger.info('activate bed last resort');
        try {
          if (this._interactingWithBed) {
            return;
          }
