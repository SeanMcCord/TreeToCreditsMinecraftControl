    ...
    if (itemEntity != null) {
      bot.collectBlock.collect(itemEntity, async (err) => {
        if (err) {
          ...
          console.log({failedToCollectItem: itemEntity, err})
          reject(err)
          return
        } else {
          resolve()
      ...
      console.log({itemNotFound: itemId})
...
export async function collectNblocks(bot, quantity, blockIds, range) {
  console.log({colleting: {quantity, blockIds}});
  await new Promise(async (resolve, reject) => {
    if (quantity == 0) {
      resolve();
      return;
...
async function getLowestCostPosition(bot, blockPositions) {
  ...
  const distances = blocks.map((blockPosition) => goalFinalMove.distanceTo(blockPosition))
  let lowest = 0;
