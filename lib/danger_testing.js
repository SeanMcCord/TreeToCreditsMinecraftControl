...
const buildCloster = async (bot, mcData) => {
  ...
  logger.info({closter: position, exactPosition: bot.entity.position, directionToCenter});
  logger.info('time to build!!!!!!!!!');
  ...
  if (buildingMaterial == null) {
    logger.info('no building material to make closter :(');
...
const otherTest = async (bot, mcData) => {
  ...
  const attackClosestBlaze = async () => {
    ...
    if (bot.pvp.target !== blaze) {
      logger.info({distanceToCloserBlaze: distance});
      if (closter) return;
      bot.pvp.attack(blaze);
      logger.info('not waiting on kill');
    }
  };
  ...
  const closterDetect = () => {
    if (bot.health < 6 && closter == false) {
      ...
      logger.info('time to closter!!!!!!!!!');
    } else if (bot.health === 20 && bot.food === 20 && closter == true) {
      closter == false;
      logger.info('closter over');
      setInterval(attackClosestBlaze, 1000);
    }
  };
